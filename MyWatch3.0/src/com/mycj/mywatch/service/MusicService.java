...
//		}
	...
	public void playByCurrentPlayingPosition(){
		if (list!=null&&list.size()>0) {
			play( MusicLoader.getMusicUriById(list.get(playingPosition).getId()));
			blePlaying();
		}
	...
	public void playByCurrentPlayingPosition(int pos ){
		if (list!=null&&list.size()>0) {
			play(MusicLoader.getMusicUriById(list.get(pos).getId()));
			blePlaying();
		}
...
//		WifiLock wifiLock = ((WifiManager) getSystemService(Context.WIFI_SERVICE)).createWifiLock(
...
//					   initMediaPlayer(R.raw.xj);
	...
	public List<Music> initMusic() {
		...
		List<Music> listMusic = MusicLoader.getMusicList();
